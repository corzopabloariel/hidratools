...
Pyrus = function (e = null, dataPYRUS = null, modify = null) {
        ...
        dataAUX.forEach( function( x ) {
            ...
            this.columnas().forEach( function( column ) {
                ...
                if( this.especificacion[ column.COLUMN ].TIPO == "TP_IMAGE" ) {
                    ...
                    if( element.e == "mp4" ) {
                        element = element.i;
                        videoURL = element == "" ? element : `${url}/${element}`;
                        ...
                        element = `<video style="width: ${w};" class="d-block mx-auto" controls>`;
                            element += `<source src="${videoURL}" type="video/mp4">`;
                            element += `Your browser does not support the video tag.`;
                        element += `</video>`;
                        element += `<p class="text-center mx-auto mt-1"><strong class="mr-1">Tipo:</strong>video MP4</p>`;
                        element += `<p class="text-center mx-auto mt-1"><strong class="mr-1 text-truncate">URL:</strong>${videoURL}</p>`;
                        element += `<p class="text-center mx-auto mt-1 d-flex justify-content-center flex-wrap align-items-center">`;
                            element += `<a href="${videoURL}" target="blank"><i class="fas fa-external-link-alt"></i></a>`;
                        element += `</p>`;
                    } else {
                        element = element.i;
                        imgURL = element == "" ? element : `${url}/${element}`;
    ...
    this.show = ( CKEDITOR_ , url , data , identifierNAME = null , identifier = null ) => {
        ...
        for( let x in this.especificacion ) {
            ...
            if( this.especificacion[x].TIPO == "TP_FECHA" ) {
                if( data[ x ] !== null ) {
                    element = dates.string( dates.convert( data[ x ] ) , this.especificacion[x].FORMAT === undefined ? [ "aaaa" , "-" , "mm" , "-" , "dd" ] : this.especificacion[x].FORMAT );
                    ...
                    rowElementos.forEach( function( e ) {
                        ...
                        if(e == "VACIO") {
                            ...
                            if( especificacion.REMOVE === undefined ) {
                                ...
                                if(this.objeto.PLACEHOLDER !== undefined && especificacion.VISIBILIDAD == "TP_VISIBLE" ) {
                                ...
                                if( ( especificacion.TIPO == "TP_FECHA" && especificacion.SIMPLE === undefined ) || ( especificacion.MULTIPLE !== undefined && especificacion.MULTIPLE ) ) {
                                    if( especificacion.VISIBILIDAD == "TP_VISIBLE" || especificacion.VISIBILIDAD == "TP_VISIBLE_FORM" )
    ...
    this.inputString = ( Object_ , names , STR_type , STR_class , OBJ_funcion = null , placeholder = "" ) => {
        ...
        if(STR_type == "file") {
            ...
            if( Object_.TIPO == "TP_IMAGE" || Object_.TIPO == "TP_BLOB" ) {
                ...
                img = `<div class="d-flex overflow-hidden flex-wrap mx-auto" style="max-width: 100%; width: ${w}; height: ${h};">`;
                    img += `<img id="src-${Arr.idElementForm}" class="mx-auto d-block border bg-dark" style="width: ${w} ; height: ${h} ; max-width: 100%;" src="" onError="this.src='${src}'" />`;
                    //img += `<img id="src-${Arr.idElementForm}-2" class="w-100 mx-auto d-block border bg-ligth" style="max-width: 50%;" src="" onError="this.src='${src}'" />`;
                img += `</div>`
    ...
    this.select = ( OBJ_elemento , names , STR_class , OBJ_funcion , OBJ_datos = null , placeholder = null ) => {
        ...
        Arr.nameElementForm += OBJ_elemento["MULTIPLE"] ? "[]" : "";
